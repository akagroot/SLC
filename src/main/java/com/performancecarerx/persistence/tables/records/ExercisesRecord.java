/**
 * This class is generated by jOOQ
 */
package com.performancecarerx.persistence.tables.records;


import com.performancecarerx.persistence.tables.Exercises;

import javax.annotation.Generated;

import org.jooq.Field;
import org.jooq.Record1;
import org.jooq.Record5;
import org.jooq.Row5;
import org.jooq.impl.UpdatableRecordImpl;


/**
 * This class is generated by jOOQ.
 */
@Generated(
    value = {
        "http://www.jooq.org",
        "jOOQ version:3.8.3"
    },
    comments = "This class is generated by jOOQ"
)
@SuppressWarnings({ "all", "unchecked", "rawtypes" })
public class ExercisesRecord extends UpdatableRecordImpl<ExercisesRecord> implements Record5<Long, String, String, Boolean, Integer> {

    private static final long serialVersionUID = 1503198431;

    /**
     * Setter for <code>public.exercises.id</code>.
     */
    public void setId(Long value) {
        set(0, value);
    }

    /**
     * Getter for <code>public.exercises.id</code>.
     */
    public Long getId() {
        return (Long) get(0);
    }

    /**
     * Setter for <code>public.exercises.name</code>.
     */
    public void setName(String value) {
        set(1, value);
    }

    /**
     * Getter for <code>public.exercises.name</code>.
     */
    public String getName() {
        return (String) get(1);
    }

    /**
     * Setter for <code>public.exercises.exercise_group_key_name</code>.
     */
    public void setExerciseGroupKeyName(String value) {
        set(2, value);
    }

    /**
     * Getter for <code>public.exercises.exercise_group_key_name</code>.
     */
    public String getExerciseGroupKeyName() {
        return (String) get(2);
    }

    /**
     * Setter for <code>public.exercises.is_deleted</code>.
     */
    public void setIsDeleted(Boolean value) {
        set(3, value);
    }

    /**
     * Getter for <code>public.exercises.is_deleted</code>.
     */
    public Boolean getIsDeleted() {
        return (Boolean) get(3);
    }

    /**
     * Setter for <code>public.exercises.ratio_profile_id</code>.
     */
    public void setRatioProfileId(Integer value) {
        set(4, value);
    }

    /**
     * Getter for <code>public.exercises.ratio_profile_id</code>.
     */
    public Integer getRatioProfileId() {
        return (Integer) get(4);
    }

    // -------------------------------------------------------------------------
    // Primary key information
    // -------------------------------------------------------------------------

    /**
     * {@inheritDoc}
     */
    @Override
    public Record1<Long> key() {
        return (Record1) super.key();
    }

    // -------------------------------------------------------------------------
    // Record5 type implementation
    // -------------------------------------------------------------------------

    /**
     * {@inheritDoc}
     */
    @Override
    public Row5<Long, String, String, Boolean, Integer> fieldsRow() {
        return (Row5) super.fieldsRow();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Row5<Long, String, String, Boolean, Integer> valuesRow() {
        return (Row5) super.valuesRow();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Field<Long> field1() {
        return Exercises.EXERCISES.ID;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Field<String> field2() {
        return Exercises.EXERCISES.NAME;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Field<String> field3() {
        return Exercises.EXERCISES.EXERCISE_GROUP_KEY_NAME;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Field<Boolean> field4() {
        return Exercises.EXERCISES.IS_DELETED;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Field<Integer> field5() {
        return Exercises.EXERCISES.RATIO_PROFILE_ID;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Long value1() {
        return getId();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public String value2() {
        return getName();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public String value3() {
        return getExerciseGroupKeyName();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Boolean value4() {
        return getIsDeleted();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Integer value5() {
        return getRatioProfileId();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public ExercisesRecord value1(Long value) {
        setId(value);
        return this;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public ExercisesRecord value2(String value) {
        setName(value);
        return this;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public ExercisesRecord value3(String value) {
        setExerciseGroupKeyName(value);
        return this;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public ExercisesRecord value4(Boolean value) {
        setIsDeleted(value);
        return this;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public ExercisesRecord value5(Integer value) {
        setRatioProfileId(value);
        return this;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public ExercisesRecord values(Long value1, String value2, String value3, Boolean value4, Integer value5) {
        value1(value1);
        value2(value2);
        value3(value3);
        value4(value4);
        value5(value5);
        return this;
    }

    // -------------------------------------------------------------------------
    // Constructors
    // -------------------------------------------------------------------------

    /**
     * Create a detached ExercisesRecord
     */
    public ExercisesRecord() {
        super(Exercises.EXERCISES);
    }

    /**
     * Create a detached, initialised ExercisesRecord
     */
    public ExercisesRecord(Long id, String name, String exerciseGroupKeyName, Boolean isDeleted, Integer ratioProfileId) {
        super(Exercises.EXERCISES);

        set(0, id);
        set(1, name);
        set(2, exerciseGroupKeyName);
        set(3, isDeleted);
        set(4, ratioProfileId);
    }
}
